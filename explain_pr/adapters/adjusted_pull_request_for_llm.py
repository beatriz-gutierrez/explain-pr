...
def adjust_patch_data_size(
    ...
    size_per_text = (
        pr_analytics.title_size + pr_analytics.description_size + sum(pr_analytics.commit_messages_size.values())
    )

    code_size = sum([file["total_size"] for file in pr_analytics.files_changes_size.values()])
    ...
    while code_size > remaining_max_code_size and remove_index < len(pr_analytics.files_changes_size):
